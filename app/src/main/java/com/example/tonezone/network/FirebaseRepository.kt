...
class FirebaseRepository {
    ...
    fun getAllTracks() {
        ...
        db.collection("Track").get().addOnSuccessListener { documents ->
...
//    }
    ...
    private fun insertTrack(track: Track) {
        if (track.preview_url != null) {
            ...
            db.collection("Track").document(trackInserting.id).set(trackInserting)
                .addOnFailureListener {
    ...
    fun getTracksOfArtist(artist: Artist): MutableLiveData<List<Track>> {
        ...
        db.collection("Track").whereArrayContains("artists", artistInTrack)
    ...
    fun getTracksOfArtist(id: String, name: String, limit: Int = 20): MutableLiveData<List<Track>> {
        ...
        val tracks = MutableLiveData<List<Track>>()
        db.collection("Track").whereArrayContains("artists", artistInTrack).limit(limit.toLong())
    ...
    fun getTracksOfPlaylist(id: String): MutableLiveData<List<Track>> {
        ...
        db.collection("Playlist").document(id).addSnapshotListener { value, _ ->
            if (value != null && value.exists()) {
                val itemIDs = convertHashMapToTrackInPlaylist(value.get("tracks") as List<*>)
                var subListItemIDs = subList(itemIDs)

                val listData = mutableListOf<Track>()
                subListItemIDs.forEach { itemIDs ->

                    db.collection("Track").whereIn("id", itemIDs).addSnapshotListener { value, _ ->
                        if (value != null) {
                            val tracksRaw = value.toObjects(Track::class.java)
                            tracksRaw.forEach { track ->
                                if (!listData.contains(track)) listData.add(track)
                            }
...
//    }
    ...
    fun getArtist(id: String): MutableLiveData<Artist> {
        ...
        db.collection("Artist").document(id).addSnapshotListener { value, error ->
            if (error != null) {
    ...
    fun getSeveralArtists(ids: List<String>): MutableLiveData<List<Artist>> {
        ...
        db.collection("Artist").whereIn("id", ids).get().addOnSuccessListener { documents ->
            if (documents != null) {
...
//    }
    ...
    fun getRecentPlaylists(user: User): MutableLiveData<GroupPlaylist> {
        ...
        db.collection("User").document(user.id).collection("History")
            .whereEqualTo("type", Type.PLAYLIST).addSnapshotListener { value, error ->
                if (value != null && !value.isEmpty()) {
                    ...
                    db.collection("Playlist").whereIn("id", playlistIDs)
    ...
    fun removeTrackFromPlaylist(playlistID: String, trackID: String) {
        ...
        db.collection("Playlist").document(playlistID).get().addOnSuccessListener { doc ->
            if (doc != null && doc.exists()) {
                val trackInPlaylist =
                    (doc["tracks"] as List<HashMap<*, *>>).find { it["id"] == trackID }
    ...
    fun addItemToSystemPlaylist(playlistID: String, tracks: List<TrackInPlaylist>) {
        val dataInserting = tracks.map {
            TrackInPlaylist(
                id = it.track.id, added_at = it.added_at
            )
        }
    ...
    fun deletePlaylist(userID: String, id: String) {
        db.collection("Playlist").document(id).delete()
    ...
    private fun createPlaylist(playlist: Playlist): Task<Void> {
        ...
        return db.collection("Playlist").document().set(playlist)
    }

    fun createPlaylist(name: String, user: User): Playlist {
        val playlist = Playlist(
            name = name, type = "playlist", owner = Owner(user.display_name, user.id), id = user.id
        )
        db.collection("Playlist").document().set(playlist).addOnSuccessListener {

            db.collection("Playlist").whereEqualTo("id", user.id)
                .addSnapshotListener { documents, _ ->
                    if (documents != null) {
                        for (doc in documents) {
                            playlist.id = doc.id
                            db.collection("Playlist").document(doc.id).set(playlist)
...
//    }
    ...
    fun getPlaylist(id: String?): MutableLiveData<Playlist> {
        ...
        if (id != null && id != "") db.collection("Playlist").document(id)
            .addSnapshotListener { value, error ->
                if (error != null) {
                    return@addSnapshotListener
                }
...
//            .set(hashMapOf("tracks" to newTrackIDs), SetOptions.mergeFields("tracks"))
    ...
    fun getSeveralPlaylist(ids: List<String>): MutableLiveData<List<Playlist>> {
        ...
        db.collection("Playlist").whereIn("id", ids).addSnapshotListener { documents, error ->
    ...
    fun getFeaturePlaylists(): MutableLiveData<List<Playlist>> {
        ...
        db.collection("System").document("SystemTopic").get().addOnSuccessListener { results ->
            if (results != null) {
                val featurePlaylistIDs = results.get("featured_playlists") as List<*>
                val listData = mutableListOf<Playlist>()

                db.collection("Playlist").whereIn("id", featurePlaylistIDs).get()
...
//    }
    ...
    fun getTracksOfAlbum(album: Album): MutableLiveData<List<Track>> {
        ...
        db.collection("Track").whereEqualTo("album", album).get().addOnSuccessListener { results ->
            if (results != null) {
    ...
    fun getAlbumsOfArtist(artist: Artist): MutableLiveData<List<Album>> {
        ...
        db.collection("Album").whereArrayContains("artists", artist).get().addOnSuccessListener {
            if (it != null) {
    ...
    fun getAlbumsOfArtist(
        artistId: String, artistName: String, limit: Int = 20
    ): MutableLiveData<List<Album>> {
        val albums = MutableLiveData<List<Album>>()

        db.collection("Track").whereArrayContains("artists", ArtistInTrack(artistId, artistName))
            .get().addOnSuccessListener { tracks ->
                if (tracks != null && !tracks.isEmpty) {

                    val artist =
                        tracks.toObjects(Track::class.java)[0].album?.artists?.find { it.id == artistId }

                    artist?.let {
                        db.collection("Album").whereArrayContains("artists", it)
                            .limit(limit.toLong()).get().addOnSuccessListener { results ->
                                if (results != null && !results.isEmpty) albums.value =
                                    results.toObjects(Album::class.java)
                            }
                    }
                }
            }
    ...
    fun getTracksOfAlbum(id: String): MutableLiveData<List<Track>> {
        ...
        db.collection("Track").whereEqualTo("album.id", id).get().addOnSuccessListener { results ->
            if (!results.isEmpty) {
                tracks.value = results.toObjects(Track::class.java)
            }
        }
    ...
    fun getAlbumReleases(): MutableLiveData<List<Album>> {
        ...
        db.collection("System").document("SystemTopic").get().addOnSuccessListener { results ->
            if (results != null) {
                ...
                db.collection("Album").whereIn("id", newReleasesIDs).get()
                    .addOnSuccessListener { documents ->
...
//                            }
    ...
    fun addUser(user: User) {
        ...
        db.collection("User").whereEqualTo("id", user.id).addSnapshotListener { documents, error ->
            if (documents != null) {
                for (doc in documents) {
                    user.id = doc.id
                    db.collection("User").document(doc.id).set(user)
                }
            }
        }
    ...
    fun getUserProfile(userID: String): MutableLiveData<User> {
        ...
        db.collection("User").document(userID).addSnapshotListener { document, error ->
    ...
    fun getFollowedObjects(userID: String): MutableLiveData<List<LibraryAdapter.DataItem>> {
        ...
        db.collection("User").document(userID).collection("Followed")
            .orderBy("pin", Query.Direction.DESCENDING).addSnapshotListener { value, _ ->
                list.clear()
                if (value != null && !value.isEmpty) {
                    ...
                    followedObjectsRaw.forEach { item ->
                        when (item.type) {
                            Type.TRACK.name -> {
                                db.collection("Track").whereEqualTo("id", item.id).get()
                            ...
                            Type.ALBUM.name -> {
                                db.collection("Album").whereEqualTo("id", item.id).get()
                                    .addOnCompleteListener { doc ->
                                        if (doc.isSuccessful) {
                            ...
                            Type.ARTIST.name -> {
                                db.collection("Artist").whereEqualTo("id", item.id).get()
                                    .addOnCompleteListener { doc ->
                                        if (doc.isSuccessful) {
                            ...
                            Type.PLAYLIST.name -> {
                                db.collection("Playlist").whereEqualTo("id", item.id).get()
                                    .addOnCompleteListener { doc ->
                                        if (doc.isSuccessful) {
                                            val playlist =
                                                doc.result.toObjects(Playlist::class.java)
                                            if (playlist.isNotEmpty() && !list.contains(
                                                    Pair(
                                                        LibraryAdapter.DataItem.PlaylistItem(
                                                            playlist[0], item.pin
                                                        ), item.pin
    ...
    fun pinObject(userID: String, id: String) {
        db.collection("User").document(userID).collection("Followed").document(id)
            .update("pin", true)
...
//        }
    ...
    fun followObject(userID: String, id: String, type: Type) {
        ...
        db.collection("User").document(userID).collection("Followed").document(id).set(savedObject)
...
//        }
    ...
    fun unfollowObject(userID: String, id: String) {
        ...
        db.collection("User").document(userID).collection("Followed").document(id).delete()
...
//                }
    ...
    fun checkObjectIsFollowed(
        userID: String, playlistID: String, type: Type
    ): MutableLiveData<Boolean> {

        val field = convertTypeToField(type)
        var isFollowed = MutableLiveData<Boolean>()

        db.collection("User").document(userID).collection("Followed").whereEqualTo("id", playlistID)
    ...
    fun checkObjectIsFollowed(
        userID: String, playlistIDs: List<String>, type: Type
    ...
    fun getLikedPlaylists(userID: String): MutableLiveData<List<Pair<Playlist, Boolean>>> {
        ...
        db.collection("User").document(userID).collection("Followed")
            .whereEqualTo("type", Type.PLAYLIST.name).addSnapshotListener { items, _ ->
                            ...
                            .addSnapshotListener { documents, _ ->
                                if (documents != null) {
                                    ...
                                    listData += playlists.map { playlist ->
                                        val isPin =
                                            (playlistWithPins.find { playlist.id == it["id"] }!!["pin"] as Boolean)
                                        Pair(playlist, isPin)
                                    }
    ...
    fun getFollowedAlbums(userID: String): MutableLiveData<List<Album>> {
        val albums = MutableLiveData<List<Album>>()
        db.collection("Followed").document(userID).addSnapshotListener { items, _ ->

            if (items != null) {
                if (items["albums"] != null) {
                    val albumIDGroups =
                        subList((items.get("albums") as List<HashMap<*, *>>).map { it["id"] })
                    val listData = mutableListOf<Album>()

                    albumIDGroups.forEach { albumIDs ->
                        db.collection("Album").whereIn("id", albumIDs)
                            .addSnapshotListener { documents, _ ->
                                if (documents != null) {
        ...
        db.collection("Followed").document(userID).addSnapshotListener { items, _ ->
            if (items != null) {
                if (items["artists"] != null) {
                    val artistIDGroups =
                        subList((items.get("artists") as List<HashMap<*, *>>).map { it["id"] })
                    val listData = mutableListOf<Artist>()
                    artistIDGroups.forEach { artistIDs ->

                        db.collection("Artist").whereIn("id", artistIDs)
                            .addSnapshotListener { documents, _ ->
                                if (documents != null) {
    ...
    fun getLikedTracks(userID: String): MutableLiveData<List<Track>> {
        val tracks = MutableLiveData<List<Track>>()
        db.collection("User").document(userID).collection("Followed")
            .whereEqualTo("type", Type.TRACK.name).addSnapshotListener { items, _ ->
                if (items != null && !items.isEmpty) {
                    val trackIDGroups =
                        subList((items.toObjects(FollowedObject::class.java).map { it.id }))
                    val listData = mutableListOf<Track>()
                    Log.i("trackABCs", "$trackIDGroups")

                    trackIDGroups.forEach { trackIDs ->
                        db.collection("Track").whereIn("id", trackIDs)
                            .addSnapshotListener { documents, _ ->
    ...
    fun getCategories(): MutableLiveData<List<Category>> {
        ...
        db.collection("Category").get().addOnSuccessListener { results ->
            if (results != null) {
    ...
    fun getPlaylistsOfCategory(categoryID: String): MutableLiveData<List<Playlist>> {
        ...
        db.collection("Category").document(categoryID).get().addOnSuccessListener { results ->
            if (results != null) {
                if (results["playlists"] != null) {
                    ...
                    playlistIDGroups.forEachIndexed { index, ids ->
                        db.collection("Playlist").whereIn("id", ids).get()
                            .addOnSuccessListener { documents ->
                                if (documents != null) {
                                    for (doc in documents) {
                                        listData.add(convertDocToPlaylist(doc))
...
//    }
    ...
    fun getDataHomeScreen(
        user: User, uiScope: CoroutineScope
    ): MutableLiveData<List<GroupPlaylist>> {
        ...
        uiScope.launch {
            db.collection("System").document("SystemTopic").get().addOnSuccessListener { results ->
                ...
                if (results != null) {
                    topic!!.keys.forEach {
                        db.collection("Playlist").whereIn("id", topic[it] as List<String>).get()
                            .addOnSuccessListener { documents ->
                                val listData = mutableListOf<Playlist>()
                                if (documents != null && !documents.isEmpty) {
                                    for (doc in documents) {
                        ...
                        db.collection("Album").whereIn("id", topic[it] as List<String>).get()
                            .addOnSuccessListener { documents ->
                                if (documents != null && !documents.isEmpty) {
                                    val albums = documents.toObjects(Album::class.java)
                                    availableList.add(
                                        GroupPlaylist(
                                            it, convertAlbumsToPlaylists(albums)
                                        )
                                    )
                                    groupPlaylists.value = availableList
                                }
    ...
    private fun getRecommendedPlaylists(
        ...
        db.collection("User").document(user.id).collection("Recommendation").get()
            .addOnSuccessListener { docs ->
                if (docs != null && !docs.isEmpty) {
                    val playlists = docs.toObjects(Playlist::class.java)

                    val mixForYouPlaylist = availableList.find { it.title == "Mix for you" }
                    if (mixForYouPlaylist == null) availableList.add(
                        GroupPlaylist(
                            "Mix for you", playlists
                        )
                    )
                    else {
                        availableList[availableList.indexOf(mixForYouPlaylist)].playlists =
                            playlists
                    }
    ...
    private fun getRecentPlaylists(
        user: User,
        _groupPlaylists: MutableLiveData<List<GroupPlaylist>>,
        availableList: MutableList<GroupPlaylist>
    ) {
        db.collection("User").document(user.id).collection("History")
            .whereEqualTo("type", Type.PLAYLIST).get().addOnSuccessListener { value ->
                if (value != null && !value.isEmpty) {
                    val result = value.toObjects(FirebaseRepository.History::class.java)
                    ...
                    db.collection("Playlist").whereIn("id", playlistIDs).get()
...
//
                                ...
                                if (recentlyPlayedPlaylists != null) {
                                    availableList[availableList.indexOf(recentlyPlayedPlaylists)].playlists =
                                        playlists
                                    _groupPlaylists.value = availableList
                                } else {
    ...
    private fun getYourArtists(
        ...
        db.collection("User").document(user.id).collection("Followed")
            .addSnapshotListener { items, _ ->
                if (items != null && !items.isEmpty) {
                    val artistIDGroups = subList(items.toObjects(FollowedObject::class.java)
                        .filter { it.type == Type.ARTIST.name }.map { it.id })
                    val listData = mutableListOf<Artist>()
                    artistIDGroups.forEach { artistIDs ->

                        db.collection("Artist").whereIn("id", artistIDs).get()
                            .addOnSuccessListener { documents ->
                                if (documents != null) {
                                    listData += documents.toObjects(Artist::class.java)
                                    ...
                                    if (yourFavoriteArtistsList != null) {
                                        availableList[availableList.indexOf(yourFavoriteArtistsList)].playlists =
                                            artists
                                        _groupPlaylists.value = availableList
                                    } else {
                                        availableList.add(group)
                ...
                if (items?.documents?.isEmpty() == true) {
                    Log.i("alo", "${items.documents}")

                    availableList.remove(availableList.find { it.title == "Your favorite artists" })
            ...
            db.collection("User").document(user.id).collection("Followed")
                .addSnapshotListener { docs, _ ->
                    if (docs != null && !docs.isEmpty) {
                        ...
                        val artistIds = subList(docs.toObjects(FollowedObject::class.java)
                            .filter { it.type == "ARTIST" }.map { it.id })

                        artistIds.forEach { ids ->
                            db.collection("Artist").whereIn("id", ids).get()
                                .addOnSuccessListener { results ->
                                    if (results != null && !results.isEmpty) {
                                        ...
                                        val sortedMap = calculateArtistsCosineSimilarity(
                                            userProfiles, itemProfiles, genres
                                        )
                                        getArtistsBaseOnRelateGenres(
                                            _groupPlaylists, availableList, sortedMap, artists
                                        )
                                    }
                                }
                        }
    ...
    private fun getArtistsBaseOnRelateGenres(
        _groupPlaylists: MutableLiveData<List<GroupPlaylist>>,
        availableList: MutableList<GroupPlaylist>,
        sortedMap: Map<String, Double>,
        artists: List<Artist>
    ) {
        if (sortedMap.isNotEmpty()) {
            db.collection("Artist").whereArrayContainsAny(
                "genres",
                sortedMap.keys.toList().subList(0, if (sortedMap.size < 4) sortedMap.size else 3)
                ...
                if (results != null && !results.isEmpty) {
                    ...
                    if (!availableList.contains(availableList.find { it.title == "Recommended artists" })) {

                        availableList.add(
                            GroupPlaylist(
                                "Recommended artists", convertArtistsToPlaylists(
                                    relateArtists.subList(
                                        0, if (relateArtists.size < 8) relateArtists.size else 8
                                    )
                                )
                            )
                        )
                        ...
                        val temp = availableList.find { it.title == "Recommended artists" }
                        availableList[availableList.indexOf(temp)].playlists =
                            convertArtistsToPlaylists(
                                relateArtists.subList(
                                    0, if (relateArtists.size < 8) relateArtists.size else 8
                                )
                            )
    ...
    private fun createItemProfile(
        artists: List<Artist>, genres: MutableList<String>
    ): Array<Array<Double>> {
        ...
        genres.forEachIndexed { genreIndex, genre ->
            artists.forEachIndexed { artistIndex, artist ->
                if (artist.genres != null && artist.genres.contains(genre)) itemProfiles[genreIndex][artistIndex] =
                    1.0
            }
        }
        return itemProfiles
    ...
    private fun calculateArtistsCosineSimilarity(
        userProfiles: DoubleArray, itemProfiles: Array<Array<Double>>, label: MutableList<String>
    ): Map<String, Double> {
        //calculate cosine similarity of each genre
        val cosineSimilarityArray = itemProfiles.map {
            cosineSimilarity(
                it.toDoubleArray(), userProfiles
            )
        }
                ...
                observer = Observer<List<Track>> { tracks ->
                    if (tracks.size == followedTracks.size) {
                        ...
                        val userProfiles = tracks.map { track ->
                            val dateLikeSong =
                                followedTracks.find { it.id == track.id }!!.date.toLong()
                        ...
                        if (similarityArray.keys.size < 10) getTracksOfArtists(similarityArray.map { it.key }).addOnSuccessListener {
                            recommendedTracks.value = it.toObjects(Track::class.java)
                        }
                        else getTracksOfArtists(similarityArray.map { it.key }
    ...
    private fun getInformationYourLikedTracks(userId: String): Task<QuerySnapshot> {
        return db.collection("User").document(userId).collection("Followed")
            .whereEqualTo("type", Type.TRACK.name).get()
    }
    ...
    private fun getYourLikedTracks(trackIds: List<String>): MutableLiveData<List<Track>> {
        ...
        listTrackIds.forEach { ids ->
            db.collection("Track").whereIn("id", ids).get().addOnSuccessListener { results ->
                if (!results.isEmpty) {
    ...
    private fun getTracksOfArtists(topArtists: List<Artist>): Task<QuerySnapshot> {
        return db.collection("Track").whereArrayContainsAny("album.artists", topArtists).get()
    }
    ...
    fun saveHistory(userID: String, trackID: String, score: Double, type: Type) {
        ...
        val newHistory =
            hashMapOf("id" to trackID, "date" to currentTime, "score" to score, "type" to type)
        db.collection("User").document(userID).collection("History").document().set(newHistory)
    }
...
//    }
    ...
    data class History(
        val id: String = "", val date: Int = 0, val score: Double = 0.0, val type: Type = Type.TRACK
    )

    fun putRecommendedTracks(userId: String) {
        db.collection("User").document(userId).collection("History")
            .whereEqualTo("type", Type.TRACK.name).get().addOnSuccessListener { docs ->
                if (docs != null && !docs.isEmpty) {
                    ...
                    if (day.isNotEmpty() && day.size > 10) {
                        ...
                        recentlyPlayedTrackIdsSublist.forEachIndexed { index, trackIds ->
                            db.collection("Track").whereIn("id", trackIds).get()
                                .addOnSuccessListener { results ->
                                    if (results != null && !results.isEmpty) {
                                        ...
                                        recentlyPlayedTracks.forEach {
                                            if (!trackData.contains(it)) trackData.add(it)
                                        }
                        ...
                        observer = Observer { tracks ->
                            if (tracks != null && tracks.size == recentlyPlayedIds.toSet()
                                ...
                                postRecommendedTracks(
                                    userId, cosineSimilarityArray.keys.toList().subList(0, 3)
                                )
    ...
    private fun postRecommendedTracks(userId: String, topArtists: List<Artist>) {
        db.collection("Track").whereArrayContainsAny("album.artists", topArtists).get()
            .addOnSuccessListener { docs ->
                ...
                if (docs != null && !docs.isEmpty) {
                    ...
                    val trackInPlaylists = recommendedTracks.map {
                        TrackInPlaylist(
                            it.id, Calendar.getInstance().get(Calendar.DATE).toString()
                        )
                    }
                    val playlist = Playlist(
                        id = "temp",
                    ...
                    db.collection("Playlist").document().set(playlist).addOnSuccessListener {
                        ...
                        db.collection("Playlist").whereEqualTo("id", playlist.id)
                            .addSnapshotListener { documents, _ ->
                                if (documents != null) {
                                    for (doc in documents) {
    ...
    private fun calculateCosineSimilarity(
        userProfiles: DoubleArray, itemProfiles: Array<Array<Double>>, artists: MutableList<Artist>
    ): Map<Artist, Double> {
        //calculate cosine similarity of each genre
        val cosineSimilarityArray = itemProfiles.map {
            cosineSimilarity(
    ...
    private fun createTrackProfiles(
        tracks: List<Track>, artists: MutableList<Artist>
    ): Array<Array<Double>> {
        ...
        artists.forEachIndexed { artistIndex, artist ->
            tracks.forEachIndexed { trackIndex, track ->
                if (track.album?.artists != null && track.album.artists.contains(artist)) itemProfiles[artistIndex][trackIndex] =
                    1.0
            }
        }
        return itemProfiles
    ...
    fun searchInFirebase(searchText: String): MutableLiveData<List<LibraryAdapter.DataItem>> {
        ...
        db.collection("Search").whereArrayContains("search_keywords", searchText).get()
    ...
    fun searchInFirebase(
        searchText: String, type: Type
    ): MutableLiveData<List<LibraryAdapter.DataItem>> {
        val searchList = MutableLiveData<List<LibraryAdapter.DataItem>>()
        db.collection("Search").whereArrayContains("search_keywords", searchText)
            .whereEqualTo("type", type.name.lowercase()).get().addOnCompleteListener {
                if (it.isSuccessful) {
                    val result = it.result.toObjects(SearchModel::class.java)
